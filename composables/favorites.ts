...
export const useFavoritesStore = defineStore('favorites', () => {
    ...
    const addFavoriteProduct = (payload: IProduct) => {
        const exists = favorite_products.value.some((item) => item.id === payload.id)
    ...
    const removeFavoriteProduct = (payload: IProduct) => {
        favorite_products.value = favorite_products.value.filter((item) => !(item.id === payload.id))
        localStorage.setItem('favorite_products', JSON.stringify(favorite_products.value))
    }

    const isFavorite = (payload: IProduct) => {
        return favorite_products.value.some((item) => item.id === payload.id)
    }
