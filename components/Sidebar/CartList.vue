<template>
    <div>
        ...
        <button @click="show = true" class="hidden lg:inline-block">
            <UChip :text="cartStore.totalPriceQuantity.quantity" size="2xl" inset :ui="{
        ...
        <button @click="show = true" class="inline-block lg:hidden">
            <UChip :text="cartStore.totalPriceQuantity.quantity" size="lg" inset :ui="{
                ...
                <div v-if="cartStore.cart_products.length > 0" class="flex flex-col gap-4">
                        ...
                        <div class="grid gap-3" style="grid-template-columns: 96px 1fr 38px;">
                            ...
                            <div class="flex flex-col gap-4">
                                ...
                                <div class="flex flex-col gap-1">
                                    <p class="font-roboto text-body3 text-gray-900"><span
                                            class="font-bold">MODEL:</span> TEST</p>
                                    <p v-if="item.variationId && item.variations"
                                ...
                                <div class="flex items-center space-x-2">
                                    <div class="flex items-center p-1 space-x-1">
                                        <button class="btn-icon-secondary square-medium rounded-md border-[1.5px]"
                                        ...
                                        <input
                                            ...
                                            type="text" :value="item.orderQuantity" :v-model="item.orderQuantity"
                                            disabled />
                                        <button class="btn-icon-secondary square-medium rounded-md border-[1.5px]"
                    ...
                    <div class="border-b pb-4 grid grid-cols-2" style="grid-template-columns: 2fr 100px;">
                        ...
                        <p class="font-saira font-bold text-h6-normal text-blue-900 text-right"> {{
                            cartStore.totalPriceQuantity.total.toFixed(2).replace('.', ',') }} €</p>
                    </div>
                    <div>
                        <NuxtLink to="/cart" class="btn-primary large uppercase block text-center">Pregledaj košaricu i
                            naruči</NuxtLink>
                    </div>
